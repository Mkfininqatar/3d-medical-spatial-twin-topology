...
st.title("🧠 Brain Underpass Engine: Live Spatial Telemetry")
st.markdown("Real-time high-frequency neural antenna tracking with automated cooldown mitigation systems.")
...
if stream_active:
    ...
    while stream_active:
        ...
        if anomaly_detected:
            # If the anomaly just started, lock the initial entry timestamp
            ...
            if elapsed_anomaly_time >= 5.0:
                # CRITICAL BREACH MET: Force action potential to safely fall below the threshold
                ...
                st.rerun()  # Forces layout sync to visually reset the slider position
            else:
                if st.session_state.cooldown_triggered:
                    alert_container.success("⚙️ **AUTOMATED MITIGATION COMPLETED:** Action potential has been throttled to a safe level.")
                else:
                    alert_container.error(
            ...
            # Reset counters if system cools down naturally or programmatically
            st.session_state.anomaly_start_time = None
            if st.session_state.cooldown_triggered:
                alert_container.success("⚙️ **AUTOMATED MITIGATION COMPLETED:** Action potential throttled safely below threshold limits.")
            else:
                alert_container.empty()
...
resting_potential_mv = st.sidebar.slider(
    ...
    min_value=-90.0, 
    max_value=-40.0, 
    value=-85.0,  # Updated to your requested -85.0 mV
    step=1.0,
    help="Default baseline potential for resting neurons"
)

# Linked via session state key for programmatic overrides
action_potential_mv = st.sidebar.slider(
    "Set Action Potential (mV)", 
    min_value=10.0, 
    max_value=60.0, 
...
# --- MANUAL OVERRIDE INTERFACE LAYER ---
if st.session_state.cooldown_triggered:
    st.sidebar.warning("⚠️ Engine Throttled due to Mitigation")
    if st.sidebar.button("🔌 Reset Engine to Standard (30.0 mV)", type="primary"):
        st.session_state.action_potential_mv = 30.0
...
if stream_active:
    ...
    while stream_active:
        ...
        # Simulating live streams with a bit of corruption/noise
        raw_batch = [random.uniform(0.0, 1.1) if random.random() > 0.05 else "corrupted_noise" for _ in range(sim_speed)]
        ...
        if anomaly_detected:
            ...
            if elapsed_anomaly_time >= 5.0:
                # 5 Seconds passed -> Force safe cooldown state
                st.session_state.action_potential_mv = float(anomaly_threshold_mv - 5.0)
                st.session_state.anomaly_start_time = None
                st.session_state.cooldown_triggered = True
                st.rerun()
